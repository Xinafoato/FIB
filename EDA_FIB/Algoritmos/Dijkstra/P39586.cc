...
void dijkstra(const G& g, int x, vector<int>& distance, vector<int>& path, int y, vector<int>& ways) {
    int n = g.size();
    ...
    while (not untracted.empty()) {
        ...
        if(not visited[u]) {
            ...
            for (int i = 0; i < int(g[u].size()); ++i) {
                ...
                if(distance[v] > distance[u] + peso) {
                    distance[v] = distance[u] + peso;
                    path[v] = u;
                    ways[v] = ways[u];
                    untracted.push(Arista(distance[v], v));
                }
                else if (distance[v] == distance[u] + peso) {
                    ways[v] += ways[u];
                }
            }
        }
...
int main() {
    ...
    while (cin >> n >> m) {
        ...
        dijkstra(g,x,distance,path,y, ways);
    
        if (distance[y] == numeric_limits<int>::max()) {
            cout << "no path from " << x << " to " << y << endl;
        }
        else {
            cout << "cost " << distance[y] << ", " << ways[y] <<" way(s)" << endl;
        }
    }

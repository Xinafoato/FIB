    ...
    void cons_vocal(vector<bool>& convob, vector<char>& convoi, vector<char>& sol, bool ant, int idx) {
        if (idx == 2*n) {
            ...
            for (int i = 1; i <= 2*n; ++i) {
                if (!convob[i]) {
                if(ant and i <= n) {   //anterior consonant i no visitat
                        sol[idx] = convoi[i];
                        convob[i] = true;
                        cons_vocal(convob, convoi, sol, false, idx+1);
                        convob[i] = false;
                    }
                    else if(not ant and i > n) {   //anterior vocal i no visitat
                        sol[idx] = convoi[i];
                        convob[i] = true;
                        cons_vocal(convob, convoi, sol, true, idx+1);
                        convob[i] = false;
                    } 
                }
    ...
    void cons_vocals(vector<char>& convoi) {
        ...
        vector<char> sol(2*n);
        cons_vocal(convob, convoi, sol, true, 0);
    }

...
/// BFS PARA ENCONTRAR  QUE HAY ///
  // revisar idea si eso
  Pos find_closest_target(const Pos& start, const set<Pos>& danger_zone, bool search_for_books) {
...
///ALGORISMO A* PARA MAGOS///
  ...
  Pos a_star_first_step(const Pos& start, const Pos& target, const set<Pos>& danger_zone) {
    ...
    while (!untracked.empty()) {
      ...
      // La primera celda válida que no es start es el primer paso óptimo
      if (current != start) {
        return current; // Devolver el primer paso óptimo
      }
  ...
  virtual void play () {
    ...
    for (int id : wizards(me())) {
      ...
      //Exploradors
      if(id %2 == 0){
        target = find_closest_target(current_pos, danger_zone, true); // libro mas cercano
        
      }
      else {
        target =find_closest_target(current_pos, danger_zone, false); // enemigo fuerza <2*mifuerza
      }
